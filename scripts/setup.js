...
${colors.bright}${colors.blue}╰─────────────────────────────────────╯${colors.reset}
  ...
  try {
    ...
    try {
      execSync('docker-compose up -d', { cwd: path.join(__dirname, '..'), stdio: 'pipe' });
    ...
    // 5. 더미 데이터 생성 여부 확인
    const seedPath = path.join(__dirname, '../scripts/db-seed.js');
    if (fs.existsSync(seedPath)) {
      log.step('더미 데이터 생성');
      try {
        execSync('node scripts/db-seed.js', {
          cwd: path.join(__dirname, '..'),
          stdio: 'inherit'
        });
...
📚 문서 확인:
   - QUICK_START.md
   - RAPID_DEVELOPMENT.md
  `);

  } catch (error) {
    log.error('초기화 중 오류 발생');
    console.error(error);
    process.exit(1);
  }
}
